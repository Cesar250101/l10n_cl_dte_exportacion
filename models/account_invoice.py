...
class Exportacion(models.Model):
    ...
    clausula_venta = fields.Many2one(
        comodel_name='aduanas.clausula_venta',
        string='Clausula de Venta',
        readonly=False,
    )
    ...
    chofer_nombre = fields.Char(string='Nombre Chofer')
    chofer_rut = fields.Char(string='Rut Chofer')
    ...
    def _totales_otra_moneda(self, currency_id, MntExe, MntNeto, IVA, TasaIVA,
        ...
        Totales['TpoCambio'] =  round(base._convert(currency_target.rate, currency_target, self.company_id, date), 4)
        if MntExe:
            if currency_id:
    ...
    def _receptor(self):
        ...
        if not commercial_partner_id.vat and not self._es_boleta() and not self.nc_boleta() and not self._es_exportacion():
        ...
        if not self._es_boleta() and not self.nc_boleta():
        ...
        if not street_recep and not self._es_boleta() and not self.nc_boleta():
        ...
        if not cmna_recep and not self._es_boleta() and not self.nc_boleta() and not self._es_exportacion():
    ...
    def _transporte(self):
        ...
        if expo.carrier_id:
            ...
            if self.chofer_rut:
                Transporte['Chofer'] = {}
                Transporte['Chofer']['RUTChofer'] = self.chofer_rut
                Transporte['Chofer']['NombreChofer'] = self.chofer_nombre[:30]
